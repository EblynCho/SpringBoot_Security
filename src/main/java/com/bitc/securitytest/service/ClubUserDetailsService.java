...
//        Optional : DB 사용 시 데이터 조회 후 데이터가 null 이 들어올 경우 오류가 발생할 수 있는 부분을 안전하게 사용하기 위한 제네릭(데이터타입)
        Optional<ClubMemberDto> result = clubMemberMapper.findByEmail(username, false);

//        DB 에 해당 사용자 정보가 있는지 확인
        if (result.isEmpty()) {
...
//        데이터베이스에 등급 권한 정보 컬럼이 있으면 필요 없는 내용
//        사용자 정보에 사용 등급 권한 설정
        member.addMemberRole(ClubMemberRole.USER);

//        로그인 인증 정보를 가지고 있는 ClubAuthMemberDto 클래스 타입의 객체 생성
//        매개변수로 DB 에서 가져온 정보를 넘겨서 사용자가 입력한 사용자 ID 를 가지고 있는 로그인 인증된 객체가 생성됨
